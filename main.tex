...
\lstset{basicstyle=\ttfamily, columns=fullflexible,%
  emph={address,bool,bytes32,contract,external,function,return,returns,struct,uint,uint8,uint32,uint256},%
  emphstyle={\bfseries}}
...
\label{sec:intro}
    ...
    may acquire resources for~computation-intensive tasks. In technical terms, Golem is designed as a~decentralized
    ...
    \item Payment for~a~single subtask may be as low as a fraction of cent (\$0.01 is roughly the cost of electricity
      consumed by an average workstation using 200 watts for 15 min, assumming the price of electricity is \$0.20 for 1 kWh).
    \end{itemize}
    ...
    payments to~a~single payee: a~compute node does not necessarily have to~be paid a~tiny sum for~each
    ...
    The last requirement for a micropayment solution is that the transaction fees should not exceed 1\% (up to an order of
    magnitude). More precisely, % in the setting of one-to-many payments in Golem this means that
    no more than 1\% of the task value should be used for cover transaction fees required for paying the compute nodes.
    Thus for example a payment for a task worth \$1 and performed by 10 compute nodes should incur no more than few cents
    of transaction fees.
    %, and for any computing node no more than 1\% of this node's remuneration
    %should be used to cover transaction fees, at least in the long run.
    %
    %Thus we are looking for~a~micropayments solution that~handles payments as small as \$0.01 with~transaction fees
    %that sum up to approximately 1\% of the task value or less.
    This requirement rules out traditional methods such as bank-based
...
\label{sec:costs}
    ...
    \begin{table}
      \begin{tabular}{p{9em}p{4em}p{7em}p{6em}p{6em}p{6em}}
        \hline
        Method & Constant cost & Cost per user& Cost for~10 users & Cost for~100 users & Cost for~$1\,000$ users \\ \hline
        Batched transfers   & $21\,000$  & $11\,000$ & $130\,000$  & $1.1$ mln  & $11.2$ mln \\ %\hline
        Subaccounts         & $21\,000$  & $8\,000$  & $100\,000$  & $840\,000$ &  $8.2$ mln \\ %\hline
        Lottery (check)     & $140\,000$ & $\log_2(n)/n \cdot 2\,700 $ & $150\,000$ & $155\,000$ & $165\,000$ \\ %\hline
...
\newcommand{\total}[2]{\widehat{#1}_{#2}}
    ...
    \begin{figure}
      \centering
      \includegraphics[scale=1.0]{prob10_80_90.pdf}
      \caption{A probability of receiving at~least 80\% or 90\% of~expected income after $n$
        lotteries with 10 participants}
      \label{fig:expected}
    \end{figure}
    
    %% Let us now examine a second question of how quickly a~lottery participant can receive her remuneration for~work.
    ...
    %% $p = 1/n$. Probability that~a~user will have to~take part in~$k$ lotteries before~winning for~the first time is
    %% equal to
    %% \begin{displaymath}
    %%         P(X = k) = \frac{1}{n} \cdot (1 - \frac{1}{n})^{k-1}
    %%     \end{displaymath}

    %% Therefore probability that~she receives payment before~this time is~above 63\%, the~probability that~she has to~wait
...
\label{sec:randomness}
    ...
    and~may decide to~quit the~protocol after deciding that~its outcome is unfavorable.\footnote{We assume
    ...
    price of~gas and~Ether, the~cost of~running the~contract is~approx \$20 per~year. The~cost of~contract initialization and
...
\label{sec:conclusion}    
    ...
    of winning instead of $1 / 1\,000$. Such a generalized lottery would be less expensive than running 10
...
\bibitem{ALARM} Merriam, Piper. ``Ethereum Alarm Clock'',
...
\bibitem{BITCOINJ} ``Working with~micropayment channels'', \textit{Bitcoinj website}, 
  \url{https://bitcoinj.github.io/working-with-micropayments}.
\bibitem{BITFEE} ``Bitcoin Transaction Fees Explained'', February 2014,
  \url{http://bitcoinfees.com/}.
\bibitem{BUTERIN} Buterin, Vitalik. ``Scalability, Part 1: Building on~Top'', \textit{Ethereum Blog}, September 17, 2014,
  \url{https://blog.ethereum.org/2014/09/17/scalability-part-1-building-top/}.
\bibitem{CHANGETIP} \textit{ChangeTip website}, \url{https://www.changetip.com/}.
\bibitem{COINTIP} ``Shutting Down Coinbase Tip Button'', \textit{The Coinbase Blog}, February 10, 2015,
  \url{https://blog.coinbase.com/2015/02/10/shutting-down-the-coinbase-tip-button/}.
\bibitem{CRYPTOHANDBOOK} Menezes, Alfred~J., et al. \textit{Handbook of Applied Cryptography}, 1997.
\bibitem{ETHEREUM} Buterin, Vitalik, et al. ``Ethereum White Paper'', updated September 30, 2015,
...
\bibitem{JAIN} Jain, Mohit, Siddhartha Lal and~Anish Mathuria. \textit{Peer-to-Peer (P2P) Micropayments: A Survey and Critical Analysis}, technical report, DA-IICT, Gandhingar, India, 2008,
...
\bibitem{RANDAO2} Qian, Yaucai. ``RANDAO repository'',
  \url{http://github.com/randao}.
\bibitem{RIVEST} Rivest, Ronald. ``Electronic Lottery Tickets as Micropayments'',
    in \textit{Financial Cryptography}, 307--314, Springer Verlag, 1997,
    \url{https://people.csail.mit.edu/rivest/pubs/Riv97b.pdf}.
  \bibitem{SOLIDITY} Reitwiessner, Christian, et al. ``Solidity: Trustless, decentralised, smart contracts for Ethereum'',
...
\bibitem{VISHNUMURTHY} Vishnumurthy, Viviek, Sangeeth Chandrakumar and~Emin G\"{u}n Sirer.
...
\bibitem{WHEELER} Wheeler, David. ``Transactions Using Bets'', in
...
\subsection{Lottery Verification}
    ...
    Finally, a~Merkle tree is~a~\textbf{labeled full binary tree} $(T, l)$ with~the~labelling function
