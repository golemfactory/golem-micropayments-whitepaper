...
\section{Ethereum}
    ...
    Golem. For example, there exist preliminary implementations of \textsc{randao} (RANdom generating Decentralized Autonomous
    Organization) which generate random numbers from users' input. Users are paid for providing inputs with
    money paid by users that consume random numbers. Of the two \textsc{randao} implementations we are aware of, the one at
    \url{https://github.com/dennismckinnon/Ethereum-Contracts/tree/master/RANDAO} is compatible with obsolete POC5
    version of Ethereum and seems to be abandoned, the other~one at \url{https://github.com/randao/randao/} is more
    up-to-date.

    Another smart contract that can be useful for our purposes is the Ethereum Alarm Clock
    (\url{http://www.ethereum-alarm-clock.com/}) that allows users to~schedule a~contract call for~a specified future block.
    ...
    In Section~\ref{sec:randao} we discuss problems with solutions that produce random values based solely on user input,
...
\label{sec:cost}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=0.6]{payments.pdf}
