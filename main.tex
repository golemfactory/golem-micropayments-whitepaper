...
\label{sec:probabilistic}
    ...
    value is getting lower as the number of attempts grows. Users may want to know how fast will their real income converge
    to the expected value and may ask questions of the kind:
    \begin{quote}
      In how many lotteries should I participate to get at least 80\% of the expected income with 90\% probability.
    \end{quote}
    For simplicity we assume that all lotteries have the same reward and the probability of winning $p$. The probability 
    that the user wins exactly $k$ lotteries out of $n$ is given by the probability mass function of the binomial distribution:
    \[
    ...
    \]
    Suppose the user participates in $n = 100$ lotteries, each with $p = 0.1$. The expected number of lotteries won is
    $p\,n = 10$. The user will get at least 80\% of the expected income if it wins in at least $8$ lotteries, 
    the probability of which is $F(8, 100, 0.1) \approx 0.79$.

    In Figure~\ref{fig:expected} we plot the values of $F(r * n, n, p)$ for $p = 0.1$ (lottery with 10 users)
    and $r = 80\%$ and $r = 90\%$. As shown in the plot, getting 90\% of the reward with probability $0.9$
    requires taking part in approximately $1\;400$ lotteries.
    \begin{figure}
      \centering
    ...
    Letus now examine how quickly a~lottery participant can receive her remuneration for~work. For simplicity let's assume
